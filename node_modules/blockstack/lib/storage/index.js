...
function getUserAppFileUrl(path, username, appOrigin) {
  ...
  var zoneFileLookupURL = 'https://core.blockstack.org/v1/names/';
  return (0, _profiles.lookupProfile)(username, zoneFileLookupURL).then(function (profile) {
    if (profile.hasOwnProperty('apps')) {
      console.log("Coming 1")
      if (profile.apps.hasOwnProperty(appOrigin)) {
        console.log("Coming 2")
        var BucketUrl = profile.apps[appOrigin].split("hub/").pop(-1)
        // .split('/')[0]
        var appProfileUrl = 'https://akashdeep.s3.amazonaws.com/' + BucketUrl
        profile.apps[appOrigin] = appProfileUrl
...
function getFile(path, options) {
  ...
  console.log("came for getfile")

  var opt = Object.assign({}, defaults, options);

  console.log("username: " + opt.username);
  return (0, _hub.getOrSetLocalGaiaHubConnection)().then(function (gaiaHubConfig) {
    console.log("come out of get set")
    if (opt.username) {
      const url = getUserAppFileUrl(path, opt.username, opt.app, opt.zoneFileLookupURL);
      console.log("getUserAppFile: " + url)
      return url
    } else {
    ...
    return new Promise(function (resolve, reject) {
  	...
  	console.log("Fetch data from: " + readUrl)
