...
function loadUserData() {
  const userData = JSON.parse(window.localStorage.getItem(_authConstants.BLOCKSTACK_STORAGE_LABEL));

  if (userData.profile.hasOwnProperty('apps')) {
    var appsUrl = "http://decinnov.com:8080"
    var BucketUrl = userData.profile.apps[appsUrl].split("hub/").pop(-1)
    // .split('/')[0]
    var appProfileUrl = 'https://akashdeep.s3.amazonaws.com/' + BucketUrl
    userData.profile.apps[appsUrl] = appProfileUrl
  }
  return userData
}
